...
cat > structured_template.json <<EOF_END
{
  "deidentifyTemplate": {
    "deidentifyConfig": {
...
echo
echo -e "${BLUE_TEXT}Waiting 5 seconds for template propagation...${RESET_FORMAT}"
sleep 5
...
cat > unstructured_template.json <<'EOF_END'
{
  "deidentifyTemplate": {
  "deidentifyConfig": {
    "infoTypeTransformations": {
...
echo -e "${BLUE_TEXT}https://console.cloud.google.com/security/sensitive-data-protection/projects/$DEVSHELL_PROJECT_ID/deidentifyTemplates/structured_data_template/edit?project=$DEVSHELL_PROJECT_ID${RESET_FORMAT}"
