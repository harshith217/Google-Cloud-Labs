...
echo "${BLUE_TEXT}${BOLD_TEXT}🔍  Attempting to automatically detect your GCP region...${RESET_FORMAT}"
export REGION=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-region])")

if [ -z "$REGION" ]; then
  echo "${YELLOW_TEXT}${BOLD_TEXT}⚠️  Could not automatically detect the region.${RESET_FORMAT}"
  echo "${BLUE_TEXT}${BOLD_TEXT}✍️  Please provide your GCP region manually below.${RESET_FORMAT}"
