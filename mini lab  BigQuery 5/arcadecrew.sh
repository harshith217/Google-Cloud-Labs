...
echo "${MAGENTA_TEXT}Creating ${MALE_TABLE_NAME} table...${RESET_FORMAT}"
...
# Create the male_customers table using SQL - without trying to detect column names
bq query --use_legacy_sql=false \
    "CREATE OR REPLACE TABLE \`${PROJECT_ID}.${DATASET_NAME}.${MALE_TABLE_NAME}\` AS 
     SELECT * 
     FROM \`${PROJECT_ID}.${DATASET_NAME}.${TABLE_NAME}\`
     WHERE UPPER(Gender) = 'MALE' OR Gender = 'Male' OR Gender = 'male'" || handle_error "Failed to create male_customers table"
